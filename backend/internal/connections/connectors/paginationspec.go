...
// buildRESTPaginator maps a connections.PaginationSpec to the matching
// httpclient.Paginator for REST-style APIs.
func buildRESTPaginator(spec *connections.PaginationSpec) (httpclient.Paginator, error) {
	switch spec.Strategy {
	case "offset":
		return &httpclient.OffsetLimitPaginator{
	...
	case "page":
		return &httpclient.PagePaginator{
			PageParam:     spec.PageParam,
			PageSizeParam: spec.PageSizeParam,
			PageSize:      spec.PageSize,
			ItemsPath:     spec.ItemsPath,
		}, nil
	case "cursor":
		return &httpclient.CursorPaginator{
...
// buildGraphQLPaginator maps a connections.PaginationSpec to the relay-style
// GraphQL paginator. GraphQL connections only support "graphqlRelay" (or no
// pagination at all).
func buildGraphQLPaginator(dataPath string, spec *connections.PaginationSpec) (httpclient.Paginator, error) {
	...
	return &httpclient.GraphQLRelayPaginator{
		DataPath:         dataPath,
		CursorVariable:   spec.GraphQLCursorVariable,
		PageSizeVariable: spec.GraphQLPageSizeVariable,
		PageSize:         spec.PageSize,
	}, nil
}

func maxPagesOf(spec *connections.PaginationSpec) int {
	if spec == nil {
		return 0
	}
